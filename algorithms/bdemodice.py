"""B-DemoDICE: DemoDICE cost discriminator as a learned reward, with
IQL-style Q / soft-Bellman V / weighted-BC actor.

Pipeline per update step
------------------------
1. Cost discriminator  : minimax BCE + WGAN-GP  →  r(s,a) = -cost_transform
2. Q  loss (Bellman)   : (r(s,a) + γ V(s') − Q(s,a))²
3. V  loss (soft Bellman): V(s) + α exp(Q_target(s,a) − V(s))
4. Actor (weighted BC) : −E[exp(Q_target − V) · log π(a|s)]
"""
...
AgentTrainState = namedtuple("AgentTrainState", "cost dual_q dual_q_target value actor")
Transition = namedtuple("Transition", "obs action reward next_obs done")


class CostNetwork(nn.Module):
    """Discriminator: high output = expert-like (s, a)."""
    obs_mean: jax.Array
    obs_std: jax.Array
...
def eval_agent(args, rng, env, agent_state):
    ...
    # --- Deterministic rollout (eval=True uses tanh(mean)) ---
    @jax.jit
    @jax.vmap
...
def make_train_step(
    args, cost_apply_fn, q_apply_fn, value_apply_fn, actor_apply_fn,
    expert_data, union_data,
):
    """Return a JIT-compatible B-DemoDICE train step.
    ...
    def _train_step(runner_state, _):
        ...
        # ------------------------------------------------------------------ #
        # 1. Update cost discriminator (identical to DemoDICE)               #
        # ------------------------------------------------------------------ #
        ...
        # ------------------------------------------------------------------ #
        # Learned reward  r(s,a) = −cost_transform  (stop-gradient)          #
        # ------------------------------------------------------------------ #
        union_cost_sg = cost_apply_fn(
            jax.lax.stop_gradient(agent_state.cost.params), uni.obs, uni.action
        )
        # Original (unnormalized) reward from cost discriminator
        r_sa = jax.lax.stop_gradient(
            -jnp.log(1.0 / (jax.nn.sigmoid(union_cost_sg) + EPS2) - 1.0 + EPS2)
        )
        #r_sa_raw = -jnp.log(1.0 / (jax.nn.sigmoid(union_cost_sg) + EPS2) - 1.0 + EPS2)
        # Center rewards per batch: removes DC offset that drives Q to large negative values.
        # (union data is mostly non-expert so raw mean ≈ −1.5 → Q ≈ −150, destabilising V↔Q loop)
        #r_sa = jax.lax.stop_gradient(r_sa_raw - r_sa_raw.mean())
        ...
        # ------------------------------------------------------------------ #
        # 4. Update V  :  V(s) + α exp(Q_target(s,a) − V(s))               #
        # ------------------------------------------------------------------ #
        # Q_target(s,a) as target for V — stop-gradient, take min of two Qs
